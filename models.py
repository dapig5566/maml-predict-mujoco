...
class Policy:
    def __init__(self, ob_dim, action_dim, hidden_dim, min_std=1e-3, max_std=1.2, memory_size=128, device_id=None):
        # #########################################################################
        # #                              test RNNSGD                              #
        # #########################################################################
        # 暂时搁置
        ...
        self.weights = []
        self.build_initial_weights(name="policy_GPU_{}".format(self.device_id))
        self.build_initial_weights(name="backup_GPU_{}".format(self.device_id))
        self.build_initial_weights(name="target_policy_GPU_{}".format(self.device_id))
...
class BaseMLP:
    ...
    def build_forward(self, x):
        with tf.variable_scope(self.name or "mlp"):
...
class ContextEncoder:
    def __init__(self, hidden_dim, embedding_dim, name=None):
        self.name = name
        self.embedding_dim = embedding_dim
        self.hidden_dim = hidden_dim
        self.context_dim = hidden_dim
        self.contextual_encoder = tf.keras.layers.CuDNNLSTM(self.hidden_dim, return_state=True, return_sequences=True, name=(name or "")+"_cu_dnnlstm")

        self.transition_encoder = tf.layers.Dense(self.embedding_dim)

    def build_forward(self, x, init_state=None):
        with tf.variable_scope(self.name or "rnn_ctx_enc"):
            x = self.transition_encoder(x)
            x = tf.nn.relu(x)
