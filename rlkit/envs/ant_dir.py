...
class AntDirEnv(MultitaskAntEnv):
    ...
    def step(self, action):
        ...
        ob = self._get_obs()
        return ob, reward, done or self.left_steps <= 0, dict(
    ...
    def reset_model(self):
        self.left_steps = 100
        return super(AntDirEnv, self).reset_model()

    def _get_obs(self):
        obs = super(AntDirEnv, self)._get_obs()
        return np.concatenate([obs, np.array([self.left_steps / self.max_steps])], axis=-1)
