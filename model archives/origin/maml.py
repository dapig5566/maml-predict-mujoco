...
class MAMLExploration:
    def __init__(self, ob_dim, action_dim, num_tasks, hidden_dim=64, discrete=True, training=True):
        ...
        self.policy = Policy(self.ob_dim, self.action_dim, self.hidden_dim)
        self.qfunc1 = Qfunction(256, "q_func1")
        self.qfunc2 = Qfunction(256, "q_func2")
        self.vfunc = Vfunction(256, "v_func")
        self.target_vfunc = Vfunction(256, "target_v_func")
        self.context_encoder = ContextEncoder(FLAGS.hidden_dim, 128)
    ...
    def build_train_op(self):
        ...
        def overwrite_vars(scope1, scope2, operator=None):

            if operator is None:
                operator = lambda x, y: y
            assert hasattr(operator, "__call__"), "operator must be callable."

            s1 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope1)
            s2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope2)
        ...
        def take_inner_steps(inp):
            ...
            for i in range(FLAGS.num_inner_loop):
                ...
                ct, _, _ = self.context_encoder.build_forward(tr)
                ct = tf.concat([tf.zeros([FLAGS.inner_batch_size, 1, self.context_encoder.context_dim]), ct[:, :-1, :]], axis=1)
                ob_ct = tf.concat([ob, ct], axis=-1)
                ac = tf.reshape(ac, [-1, self.action_dim])

                x = self.policy.build_mlp(ob_ct, weights=w)
                ...
                ct = tf.reshape(ct, [-1, self.context_encoder.context_dim])
                saz = tf.concat([ob, ac, ct], axis=-1)
                min_q = tf.minimum(self.qfunc1.build_forward(saz), self.qfunc2.build_forward(saz))
        ...
        def train_value_func(inp):
            ...
            cts, _, _ = self.context_encoder.build_forward(trs)
            ...
            sz2 = tf.reshape(sz2, [-1, self.ob_dim + self.context_encoder.context_dim])
            q1 = self.qfunc1.build_forward(saz)
            q2 = self.qfunc2.build_forward(saz)
            target_v = self.target_vfunc.build_forward(sz2)
            ...
            x = self.policy.build_mlp(ob_ct, weights=self.policy.weights[0])
            ...
            bc_z = tf.broadcast_to(task_z, [tf.shape(new_actions)[0], tf.shape(new_actions)[1], self.context_encoder.context_dim])
            bc_o = tf.broadcast_to(o, [tf.shape(new_actions)[0], tf.shape(new_actions)[1], self.ob_dim])
            sz = tf.concat([o, task_z], axis=-1)
            sz = tf.reshape(sz, [-1, self.ob_dim + self.context_encoder.context_dim])
            saz = tf.concat([bc_o, new_actions, bc_z], axis=-1)
            saz = tf.reshape(saz, [tf.shape(new_actions)[0], tf.shape(new_actions)[1], self.ob_dim + self.action_dim + self.context_encoder.context_dim])
            q1 = self.qfunc1.build_forward(saz)
            q2 = self.qfunc2.build_forward(saz)
            min_q = tf.minimum(q1, q2)
            v = self.vfunc.build_forward(sz)
        ...
        # forward
        weights, weights_bak = self.policy.weights[:2]
        self.save_origin = assign_vars(weights_bak, weights)
        self.restore_origin = assign_vars(weights, weights_bak)

        self.initialize_target = tf.group(overwrite_vars("target_v_func", "v_func"),
                                          overwrite_vars("target_policy", "policy"))
        soft_update = lambda x, y: (1 - self.tau) * x + self.tau * y
        self.update_target_vfs_op = overwrite_vars("target_v_func", "v_func", operator=soft_update)
        self.update_target_policy_op = overwrite_vars("target_policy", "policy", operator=soft_update)

        obs = tf.reshape(self.obs, [1, -1, self.ob_dim])
        acs = tf.reshape(self.actions, [1, -1, self.action_dim])
        rws = tf.reshape(self.rews, [1, -1, 1])
        cts = tf.reshape(self.context, [1, -1, self.context_encoder.context_dim])
        ob_ct = tf.concat([obs, cts], axis=-1)
        trs = tf.concat([obs, acs, rws], axis=-1)

        # task_z, _, _ = self.context_encoder.build_forward(trs)
        # task_z = tf.concat([tf.zeros([FLAGS.inner_batch_size, 1, self.context_encoder.context_dim]), task_z[:, :-1, :]], axis=1)

        # saz = tf.concat([obs, acs, task_z], axis=-1)

        # q1 = self.qfunc1.build_forward(saz)
        # q2 = self.qfunc2.build_forward(saz)
        # min_q = tf.minimum(q1, q2)

        x = self.policy.build_mlp(ob_ct, weights=weights)
        self.forward_context, self.forward_h_state, self.forward_c_state = self.context_encoder.build_forward(trs, init_state=[self.init_h, self.init_c])
        self.forward_action, distribution = sample_from(x, 1, discrete=self.discrete)
        self.forward_mean, self.forward_log_std = tf.squeeze(x[0]), tf.squeeze(x[1])
        # self.forward_advantage = tf.squeeze(min_q)

        # update
        tensors = [self.means, self.log_stds]
        means, logstds = [tf.reshape(i, [FLAGS.inner_batch_size * FLAGS.inner_time_steps, -1]) for i in tensors]
        ...
        ads = tf.reshape(self.advs, [FLAGS.inner_batch_size * FLAGS.inner_time_steps])
        trs = tf.concat([obs, acs, rws], axis=-1)
        cts, _, _ = self.context_encoder.build_forward(trs)
        cts = tf.concat([tf.zeros([FLAGS.inner_batch_size, 1, self.context_encoder.context_dim]), cts[:, :-1, :]], axis=1)
        ob_ct = tf.concat([obs, cts], axis=-1)
        ...
        cts = tf.reshape(cts, [-1, self.context_encoder.context_dim])
        ...
        if self.training:
            ...
            elems = (self.context, self.transitions, self.embedding)
            result = tf.map_fn(train_value_func, elems=elems, parallel_iterations=FLAGS.value_meta_batch_size)
            qf_loss, vf_loss, ct_emb = result
            self.qf_loss = qf_loss = tf.reduce_mean(qf_loss)
            self.vf_loss = vf_loss = tf.reduce_mean(vf_loss)
            self.get_embedding = tf.stack(ct_emb)
            qf_kl_loss = qf_loss
            self.train_ctx_enc = tf.train.AdamOptimizer(FLAGS.value_lr).minimize(qf_kl_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="rnn_ctx_enc"))
            self.qf1_op = tf.train.AdamOptimizer(FLAGS.value_lr).minimize(qf_kl_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="q_func1"))
            self.qf2_op = tf.train.AdamOptimizer(FLAGS.value_lr).minimize(qf_kl_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="q_func2"))
            self.vf_op = tf.train.AdamOptimizer(FLAGS.value_lr).minimize(vf_loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="v_func"))
    ...
    def _train_policy(self, feed_dict):
        ...
        session.run(self.update_target_policy_op)
        self.adjust_coeff(mean_kls)
        return ent

    # def compute_adv(self, task_id, observations, actions, reward):
    #     session = tf.get_default_session()
    #     feed_dict = dict(zip([self.obs, self.actions, self.rews], [observations, actions, reward]))
    #     adv = session.run(self.forward_advantage, feed_dict=feed_dict)
    #     return adv
